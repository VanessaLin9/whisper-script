...
4. 選提示詞，按「清洗」。工具在會議資料根目錄的 `.llm_jobs/inbox` 建立小份 job JSON（路徑與 hash），並依 SRT 把長會議切成約 10 分鐘的核心區段。剪貼簿只放 job 路徑。沒有可解析的 SRT 會停止，不會把整份逐字稿貼進聊天。
5. Agent 把清洗稿與 result JSON 寫入 `.llm_jobs/outbox` 後，按「匯入清洗稿」。工具核對 job、來源 hash 與輸出 hash。縮短超過 20% 會擋下，不會覆寫既有稿件。
6. 切換原始／預清洗／清洗稿檢查內容，再按「已檢查，確認清洗稿」。之後可建立會議記錄工作。匯入的是 coverage map 與草稿，不會改寫清洗稿，也不會發布 Notion。
