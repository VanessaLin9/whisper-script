...
2026-09-17 / v0.3。原生 SwiftUI + 現有 Python service，無 localhost server、web hosting、OAuth 或付費 API。Python 透過 stdin 接收單一 JSON 請求，stdout 回傳逐行進度與最終結果；不透過 shell 拼接使用者參數。

## 使用範圍

原生 App 可拖曳匯入或選取單一音檔、確認錄音時間、瀏覽既有會議、轉錄、預清洗、取消／續跑、建立 clean job、匯入完整清洗稿、預覽與內容確認、建立 notes job，以及匯入本機會議記錄草稿。預設多語言 medium、zh。ASR 不保證字體為繁體；繁體校正與專有名詞保真交由後續 LLM 清洗。

內建錄音、批次佇列、Drive GUI、LLM API、Notion 發布尚未實作。使用者可繼續使用語音備忘錄；現有 shell 入口仍保留。

## 人工訂正與會議名稱

- 會議名稱旁的鉛筆按鈕可修改顯示名稱（1–200 字）。清單、搜尋和新 job 立即使用新名稱；資料夾、原音檔、artifact 檔名與會議識別保持不變。
...
- 字幕訂正與 TXT 訂正分開保存，不會推測段落對應後自動覆蓋對方。clean job 同時引用目前 SRT 與 TXT；若文字不同，以訂正 TXT 為主要語意來源。
...
原始 `*_pipeline_state.json` 與 CLI 路徑保持不變；CLI 不會自動採用 GUI 訂正版。GUI job 明確引用使用中的訂正版路徑與 hash，不進行破壞性遷移。

## 本機 LLM jobs

- 固定 queue 位於 `<output_root>/.llm_jobs/{inbox,outbox,work,archive}`。目錄模式為 `0700`，Desk 產生的 request、segments、manifest 與保存草稿模式為 `0600`。
...
- `desktop/MeetingDesk.swift`：原生視窗、檔案拖曳／選擇、Finder、編輯失敗內容複製與背景 Python Process。
...
- `src/desktop/jobs.py`：私有 inbox/outbox contract、SRT core/context 切段、job/result hash 驗證。
...
- `desktop_state.json`：title、status、attempts、raw/prepared hashes、profile、prompt hash、`handoffs.clean/notes`、quality、notes draft pointer。
- `.desktop.lock`：flock 鎖檔，每場會議同時只有一個 GUI mutation。鎖檔可以存在，鎖在 process 結束後自動釋放。
- `<output_root>/.llm_jobs/`：私有 request、segments、outbox 與保留的 archive 位置。不能提交 Git。
- `notes_drafts/notes-<uuid>.md`：匯入後的本機會議記錄草稿版本。不能提交 Git，也不代表已發布。
...
5. clean job 必須選定有本機 note 的 profile，並記錄 input/raw/SRT/profile/segments hashes；缺本機 note 或合法 SRT 時停止。
6. 匯入清洗稿需匹配目前 job 與所有來源 hashes、非空、縮短不超過 20%；以人工訂正稿（沒有才用 prepared）字元數為主要長度基準。檢查通過後仍是 `pending_review`，不是語意保真的自動證明。
7. 使用者對照檢查後才設為 `passed`，保存 reviewer/time；notes job 建立與匯入重新檢查 input/cleaned/SRT/profile/spec hashes。檔案外部修改會失去已確認狀態。
8. 已存在的 cleaned 不覆寫。可選取同一個 cleaned 檔做重新驗證／採納 legacy 產物；若匯入不同檔案，會另存人工版本目錄並更新目前路徑，舊版仍保留。
9. Notion 明確保留 pending；notes job 只引用 repo 的 meeting-summary-spec 路徑與 hash，要求 coverage、證據、決議／提案區分，不授權遠端發布。
