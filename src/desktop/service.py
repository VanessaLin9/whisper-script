...
class DesktopService:
    ...
    def row(self, folder: Path) -> dict:
        ...
        quality_passed = (quality.get("status") == "passed" and paths["cleaned"].exists()
        ...
        if quality.get("status") in {"stale", "failed"} and paths["cleaned"].exists():
            status = "內容已變更 · 待重新檢查"
        clean_job = state.get("handoffs", {}).get("clean", {})
        clean_job_status = clean_job.get("status", "")
        if clean_job_status == "queued" and Path(clean_job.get("result_path", "")).is_file():
            clean_job_status = "done"
        if clean_job_status == "queued" and not paths["cleaned"].exists():
            status = "清洗工作已建立"
        if clean_job_status == "done" and not paths["cleaned"].exists():
            status = "清洗結果待匯入"
        notes_job = state.get("handoffs", {}).get("notes", {})
        notes_job_status = notes_job.get("status", "")
        if notes_job_status == "queued" and Path(notes_job.get("result_path", "")).is_file():
            notes_job_status = "done"
        notes = state.get("notes", {})
        notes_path = (folder / notes.get("path", "missing")).resolve()
        notes_ready = (notes_path.parent == (folder / "notes_drafts").resolve()
                       and notes_path.is_file() and file_hash(notes_path) == notes.get("sha256"))
        if notes_job_status == "queued" and status == "清洗已確認":
            status = "會議記錄工作已建立"
        if notes_job_status == "done" and status == "清洗已確認":
            status = "會議記錄草稿待匯入"
        if notes_ready and notes_job_status == "imported":
            status = "會議記錄草稿已匯入"
        ...
        return {"id": str(folder), "title": state.get("title", fallback_title),
                ...
                "cleaned": paths["cleaned"].exists(), "reviewed": quality_passed,
                "clean_job_status": clean_job_status, "notes_job_status": notes_job_status,
                "notes": notes_ready}
    ...
    def save_edit(self, value: str, kind: str, token: str, *, text=None, cues=None) -> dict:
        ...
        with meeting_lock(folder):
            ...
            if target == "cleaned":
                ...
                state["status"] = "review"
                if state.get("handoffs", {}).get("notes"):
                    state["handoffs"]["notes"]["status"] = "stale"
            else:
                if state.get("handoff"):
                    state["handoff"]["status"] = "stale"
                for handoff in state.get("handoffs", {}).values():
    ...
    def handoff_preview(self, value: str, profile: str, summary: bool = False) -> dict:
        """Plan a handoff without creating locks, queue paths, segments, or state."""
        folder = self.folder(value)
        base_paths = self.paths(folder)
        if not base_paths["prepared"].is_file():
            raise ValueError("尚未有預清洗稿，無法預覽 handoff。")
        manifest = read_json(base_paths["prepared"].with_suffix(".txt.manifest.json"))
        if (manifest.get("source_sha256") != file_hash(base_paths["raw"])
                or manifest.get("output_sha256") != file_hash(base_paths["prepared"])):
            raise ValueError("預清洗稿與來源／manifest 不一致。請先檢查既有檔案。")
        state = read_json(folder / STATE_NAME)
        paths = self.effective(folder, state)
        ...
        profile_path = Path(selected.get("local_path", ""))
        if not profile_path.is_file():
            raise ValueError("此提示詞尚未同步到本機，請先同步提示詞筆記。")
        stage = "notes" if summary else "clean"
        if summary and (state.get("quality", {}).get("status") != "passed"
                        or not self.quality_matches(state["quality"], paths)):
            raise ValueError("請先匯入清洗稿並完成內容檢查。")
        if not summary and not paths["srt"].is_file():
            raise ValueError("找不到 SRT，無法建立固定切段的清洗工作。")
        specification = self.repo / "docs" / "meeting-summary-spec.md"
        if summary and not specification.is_file():
            raise ValueError("找不到會議記錄 coverage 規格。")

        job_id = new_job_id(stage)
        stem = base_paths["raw"].name.removesuffix("_transcription.txt")
        expected_name = f"{stem}_meeting_notes.md" if summary else f"{stem}_transcription_cleaned.txt"
        result_dir = queue_root(self.root) / "outbox" / job_id
        request_path = queue_root(self.root) / "inbox" / f"{job_id}.json"
        plan = plan_srt_segments(paths["srt"]) if not summary else None
        job = {
            "job_id": job_id, "stage": stage, "status": "planned",
            "request_path": str(request_path), "input_path": str(paths["cleaned"] if summary else paths["input"]),
            "input_hash": file_hash(paths["cleaned"] if summary else paths["input"]),
            "profile": profile, "profile_hash": file_hash(profile_path),
            "expected_output": str(result_dir / expected_name),
            "result_path": str(result_dir / "result.json"),
            "segment_count": plan["segment_count"] if plan else 0,
        }
        if summary:
            job["specification_path"] = str(specification)
            job["specification_hash"] = file_hash(specification)
        else:
            job["timeline_path"] = str(paths["srt"])
            job["timeline_hash"] = plan["timeline_sha256"]
        return {"dry_run": True, "job": job, "segment_plan": plan,
                "note": "此 job_id 僅供預覽；正式建立時會產生新的 job_id。"}

    def handoff(self, value: str, profile: str, summary: bool = False, dry_run: bool = False) -> dict:
        if dry_run:
            return self.handoff_preview(value, profile, summary)
        folder = self.folder(value)
        with meeting_lock(folder):
            self.prepare(folder)
            ...
            state = read_json(folder / STATE_NAME)
            if not summary:
                if not paths["srt"].is_file():
                    raise ValueError("找不到 SRT，無法建立固定切段的清洗工作。")
                job_id = new_job_id("clean")
                manifest = segment_srt(self.root, job_id, paths["srt"])
                stem = self.paths(folder)["raw"].name.removesuffix("_transcription.txt")
                record = create_job(
                    self.root, folder, self.row(folder)["title"], "clean", profile, selected["label"],
                    Path(note_path), paths["input"], f"{stem}_transcription_cleaned.txt",
                    srt_path=paths["srt"], segments_manifest=manifest, job_id=job_id,
                )
                record.update(raw_hash=file_hash(paths["raw"]), timeline_hash=file_hash(paths["srt"]),
                              input_path=str(paths["input"]), prompt_hash=record["profile_hash"])
                state.setdefault("handoffs", {})["clean"] = record
                # Preserve the old key as a compatibility pointer for existing readers.
                state["handoff"] = dict(record)
                state.update(profile=profile, prompt_hash=record["profile_hash"])
                atomic_json(folder / STATE_NAME, state)
                return {"job": record, "path": record["request_path"],
                        "files": [record["request_path"], str(manifest)], "meeting": self.row(folder)}

            if not self.quality_matches(state.get("quality", {}), paths) or state.get("quality", {}).get("status") != "passed":
                raise ValueError("請先匯入清洗稿並完成內容檢查。")
            specification = self.repo / "docs" / "meeting-summary-spec.md"
            ...
            stem = self.paths(folder)["raw"].name.removesuffix("_transcription.txt")
            record = create_job(
                self.root, folder, self.row(folder)["title"], "notes", profile, selected["label"],
                Path(note_path), paths["cleaned"], f"{stem}_meeting_notes.md",
                srt_path=paths["srt"] if paths["srt"].is_file() else None, spec_path=specification,
            )
            record.update(cleaned_hash=file_hash(paths["cleaned"]), input_path=str(paths["cleaned"]),
                          timeline_hash=file_hash(paths["srt"]), prompt_hash=record["profile_hash"])
            state.setdefault("handoffs", {})["notes"] = record
            state.update(profile=profile, prompt_hash=record["profile_hash"])
            atomic_json(folder / STATE_NAME, state)
            return {"job": record, "path": record["request_path"],
                    "files": [record["request_path"]], "meeting": self.row(folder)}

    def list_jobs(self) -> dict:
        return {"jobs": list_jobs(self.root)}

    def validate_job(self, request_path: str) -> dict:
        return {"job": validate_job(self.root, Path(request_path))}

    def import_cleaned(self, value: str, source: str | None = None) -> dict:
        folder = self.folder(value)
        with meeting_lock(folder):
            self.prepare(folder)
            paths = self.effective(folder)
            state = read_json(folder / STATE_NAME)
            current_job = state.get("handoffs", {}).get("clean")
            completed = None
            if current_job:
                source_path, completed, job = read_completed_result(current_job)
                current_input = paths["input"].resolve()
                job_input = Path(job["input"]["path"]).resolve()
                current_timeline = paths["srt"].resolve()
                job_timeline = Path(job.get("timeline", {}).get("path", "")).resolve()
                if (current_input != job_input or file_hash(current_input) != job["input"]["sha256"]
                        or current_timeline != job_timeline
                        or file_hash(current_timeline) != job.get("timeline", {}).get("sha256")):
                    current_job["status"] = "stale"
                    atomic_json(folder / STATE_NAME, state)
                    raise ValueError("目前逐字稿或時間軸已變更，請重新建立清洗工作。")
                if source is not None and Path(source).expanduser().resolve() != source_path:
                    raise ValueError("請從目前 job 的固定 outbox 匯入結果。")
            else:
                if not source:
                    raise ValueError("尚未建立清洗工作，也沒有指定可相容匯入的清洗稿。")
            ...
            if current_job:
                current_job.update(status="imported", output_hash=completed["output_sha256"],
                                   imported_at=datetime.now(TZ).isoformat())
                state["handoff"] = dict(current_job)
            if state.get("handoffs", {}).get("notes"):
                state["handoffs"]["notes"]["status"] = "stale"
            atomic_json(folder / STATE_NAME, state)
        return {"meeting": self.row(folder)}
    ...
    def import_notes(self, value: str) -> dict:
        ...
        with meeting_lock(folder):
            ...
            state = read_json(folder / STATE_NAME)
            paths = self.effective(folder, state)
            record = state.get("handoffs", {}).get("notes")
            if not record:
                raise ValueError("尚未建立會議記錄工作。")
            if state.get("quality", {}).get("status") != "passed" or not self.quality_matches(state["quality"], paths):
                record["status"] = "stale"
                atomic_json(folder / STATE_NAME, state)
                raise ValueError("清洗稿尚未確認或已變更，請重新建立會議記錄工作。")
            source, result, job = read_completed_result(record)
            current_cleaned = paths["cleaned"].resolve()
            job_input = Path(job["input"]["path"]).resolve()
            if current_cleaned != job_input or file_hash(current_cleaned) != job["input"]["sha256"]:
                record["status"] = "stale"
                atomic_json(folder / STATE_NAME, state)
                raise ValueError("清洗稿已變更，請重新建立會議記錄工作。")
            text = source.read_text(encoding="utf-8-sig")
            if not text.strip():
                raise ValueError("會議記錄草稿不可空白。")
            directory = folder / "notes_drafts"
            directory.mkdir(exist_ok=True, mode=0o700)
            directory.chmod(0o700)
            target = directory / f"notes-{uuid4().hex}.md"
            exclusive_write_text(target, text)
            target.chmod(0o600)
            notes = {"path": str(target.relative_to(folder)), "sha256": file_hash(target),
                     "job_id": record["job_id"], "saved_at": datetime.now(TZ).isoformat()}
            state["notes"] = notes
            state.setdefault("notes_history", []).append(notes)
            record.update(status="imported", output_hash=result["output_sha256"],
                          imported_at=datetime.now(TZ).isoformat())
            atomic_json(folder / STATE_NAME, state)
        return {"meeting": self.row(folder), "path": str(target)}
    ...
    def preview(self, value: str, kind: str) -> dict:
        if kind not in {"raw", "prepared", "corrected", "cleaned", "srt", "notes"}:
            raise ValueError("不支援的預覽類型。")
        folder = self.folder(value)
        state = read_json(folder / STATE_NAME)
        paths = self.effective(folder, state)
        if kind == "notes":
            record = state.get("notes", {})
            path = (folder / record.get("path", "missing")).resolve()
            if (path.parent != (folder / "notes_drafts").resolve() or not path.is_file()
                    or file_hash(path) != record.get("sha256")):
                return {"text": "此階段尚未產生檔案。", "path": "", "editable": False}
            return {"text": path.read_text(encoding="utf-8"), "path": str(path), "editable": False}
        path = paths[kind]
        if not path.is_file():
...
def dispatch(service: DesktopService, request: dict, cancellation=None, progress=lambda value: None) -> dict:
    ...
    if action == "handoff": return service.handoff(
        request["folder"], request["profile"], request.get("summary", False), request.get("dry_run", False))
    if action == "list_jobs": return service.list_jobs()
    if action == "validate_job": return service.validate_job(request["request_path"])
    if action == "import_cleaned": return service.import_cleaned(request["folder"], request.get("path"))
